...
* Visibility Backlog — first prioritized backlog issued as part of HV-VCM-001 (5 items, per HV-P-006): publish structured data, publish omakase page, remove `/sushi-page-2/`, remove `/hello-world/`, correct third-party naming (owner action required). **Status 22 juli 2026:** omakase/menu pages (HV-INT-002) live and verified; `/sushi-page-2/` and `/hello-world/` no longer in sitemap (removed); structured data (HV-INT-001) still not on site; third-party naming correction still open. Additional, unplanned fixes shipped same day: tracking infrastructure (GTM confirmed pre-existing + GA4 connected by Kelvin, consent banner, 30+ event labels), non-functional catering form replaced with a working `wp_mail()` handler, missing Private Dining CTA added, missing directions link added.
* Measurement baseline — partial. HV-MP-001 (Measurement Plan) established; first registers instantiated: HV-BL-001 (baseline — AI/search/external-source data complete from HV-IV-003/004/001/006, website/local/business data still needs Search Console, GA4, Google Business Profile, and Guestplan exports), HV-IR-001 (Intervention Register — 2 records, both Pending Implementation), HV-TS-001 (Test Scenario Register — round 0 baseline from HV-IV-003/004), HV-AR-001 (Attribution Register — template only, requires Guestplan configuration Kelvin must do). See `solutions/visibility/`.
* Visibility Graph.
* Operational ownership model.
* Validation thresholds — verdict scale defined in HV-MP-001 §13 (Earned/Provisionally Earned/Inconclusive/Not Earned/Harmful); not yet applied to any real result since HV-INT-001/002 are not live.
* HV-DB-001 — Owner View dashboard published as a standalone page (see `solutions/visibility/HV-DB-001-visibility-dashboard.md` for the link and summary), showing round 0 baseline honestly as mostly "Pending"/"Needs data" rather than fabricated scores.
