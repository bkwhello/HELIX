...
| `catering_enquiry` productieverificatie (observations/O-028.md) | **Production Verified** — kernevents 2026-W34-brief Prioriteit 3: **6/6** (events, geen lagen); first-party-meetketengate **GESLOTEN**; case blijft Open | 26 sep 2026 | Website-commit `106016c` (Optie A: eenmalige serverreceipt) gedeployd; hashes na deploy gelijk aan de commit; vervalste/parameter-only succes-URL's → 302 naar schone pagina. Lokale runtime-test door Case Owner **gewaived** (geen runtime-bewijs). Eén productie-inzending: geaccepteerd, Nederlandse succesmelding, automatische reveal, precies één `catering_enquiry`-entry in de dataLayer zonder formulier-PII. GTM Preview: tag vuurde eenmaal. Gepubliceerde **Versie 10** bevat tag + Custom Event-trigger (`G-C29ZMF288W`, geen extra parameters), zelfstandig bevestigd via `gtm.js`. Test buiten Preview: GA4 Realtime toonde `catering_enquiry` (count 2 = alleen venstertelling, niet als twee niet-Preview-inzendingen geclaimd). EN-template-succesmelding niet runtime-geverifieerd; geen attributie (OC-007), key-event- of duplicate-claim; `/catering/`-routing blijft open |
