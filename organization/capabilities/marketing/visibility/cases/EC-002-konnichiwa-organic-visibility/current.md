...
Last updated: 25 July 2026 (Case-owner decision recorded, decisions/DD-017: Kelvin Wong issued **ESTABLISHED WITH CONDITIONS** for DQ-001's Candidate Organizational Diagnosis, OD-001, accepting all seven gate conditions verbatim. `dq_001_diagnosis_established` is now `true`; `dq_001_establishment_decision`: Established With Conditions. `diagnosis_established` becomes `true`, explicitly scoped to DQ-001 only. DQ-002, DQ-004, DQ-005, and DQ-007 remain not established and not started; DQ-003 and DQ-006 remain Not Authorized. Design, Transformation, and external changes remain unauthorized.).
...
diagnosis_established_scope: DQ-001 only
...
next_action: Select the next conditionally authorized diagnosis investigation (DQ-002, DQ-004, DQ-005, or DQ-007) or prepare a separate Design Authorization Gate limited to OD-001 — not yet chosen between
```
...
**DQ-001 is now Established With Conditions** (decisions/DD-017, Case-Owner Decision, 25 July 2026). Two independent next actions are open; this decision does not choose between them:

**Option A — select the next conditionally authorized diagnosis investigation**, one at a time:
...
Each investigation must open under its own containment conditions as recorded in decisions/DD-016 (Phase 5 Candidate Diagnosis Scope and the Case-Owner Decision's Question-Specific Rules) and must be separately gated (its own DD-0##) before establishment.

**Option B — prepare a separate Design Authorization Gate limited to OD-001**, if Kelvin wishes to move toward a design response for the established DQ-001 diagnosis specifically. This would be a new, distinct gate — no Design work is authorized by this decision itself.
...
**Not authorized by this decision:** any content, page, schema, GBP, or review action based on OD-001's finding; Design; Transformation; any SEO or marketing intervention; or any external system change — all remain separately gated and `false`.
