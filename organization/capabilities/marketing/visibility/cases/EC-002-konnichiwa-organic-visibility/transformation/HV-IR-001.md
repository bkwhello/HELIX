...
| Confidence | Hoog voor "de tag vuurt nu af". **`reservation_click`: bijgewerkt 26 augustus 2026 (observations/O-018.md) — Production Verified**, via een gerichte testklik en GTM/GA4-reconciliatie (zie O-018 voor het volledige bewijs). **`phone_click`: bijgewerkt 27 augustus 2026 (observations/O-019.md) — Production Verified**, via GTM Versie 6 ("GA4 phone_click tracking"), Preview-gedragsbewijs en GA4 Realtime-ontvangst (zie O-019 voor het volledige bewijs en de expliciet vastgelegde bewijsgrenzen). `reservation_widget_open` en de overige losse events uit HV-MP-001 §7 blijven ongewijzigd: nog niet gemeten of ze daadwerkelijk correct doorkomen — vereisen elk nog een eigen gerichte testklik. **Dit sluit de bredere first-party-meetketen niet af** (measurement/2026-W34-visibility-brief.md, Prioriteit 3, zes vereiste events) — twee van de zes (`reservation_click`, `phone_click`) zijn nu Production Verified; de overige vier blijven in hun bestaande status. |
