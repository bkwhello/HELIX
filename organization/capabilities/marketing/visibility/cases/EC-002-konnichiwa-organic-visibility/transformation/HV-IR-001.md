...
| Confidence | Hoog voor "de tag vuurt nu af". **`reservation_click`: bijgewerkt 26 augustus 2026 (observations/O-018.md) — Production Verified**, via een gerichte testklik en GTM/GA4-reconciliatie (zie O-018 voor het volledige bewijs). **`phone_click`: bijgewerkt 27 augustus 2026 (observations/O-019.md) — Production Verified**, via GTM Versie 6 ("GA4 phone_click tracking"), Preview-gedragsbewijs en GA4 Realtime-ontvangst (zie O-019 voor het volledige bewijs en de expliciet vastgelegde bewijsgrenzen). **`directions_click`: bijgewerkt 27 augustus 2026 (observations/O-020.md) — Production Verified**, via GTM Versie 7 ("GA4 directions_click tracking"), Preview-gedragsbewijs (geen cross-fire met `reservation_click`/`phone_click`, regressiecheck geslaagd) en GA4 Realtime-ontvangst (zie O-020 voor het volledige bewijs en de expliciet vastgelegde bewijsgrenzen). **`reservation_widget_open`: bijgewerkt 28 augustus 2026 (observations/O-021.md, Test 5) — Production Verified.** Drie GTM Preview-tests (0/1/2 daadwerkelijke widget-openingen → 0/1/2 `reservation_widget_open`-events, exact overeenkomend) bevestigden dat de listener (`Listener - Guestplan widget open` Custom HTML → `Reservation widget open` Custom Event trigger → `GA4 - Event - reservation_widget_open`) correct werkt in Preview. Een daaropvolgende schone, niet-Preview productietest (Test 4) toonde echter géén GA4-ontvangst van `reservation_widget_open`, terwijl `reservation_click` in datzelfde venster tweemaal wél aankwam. **Root cause bevestigd:** de vier betrokken objecten (`Listener - Guestplan widget open`, `Reservation widget open`-trigger, `GA4 - Event - reservation_widget_open`-tag, `Window Loaded - All Pages`-trigger) bestonden uitsluitend in de ongepubliceerde GTM-werkruimte — GTM Preview leest altijd de werkruimte, productieverkeer liep nog op de gepubliceerde **Versie 7**, die deze objecten niet bevatte. Na een read-only pre-publish audit (PASS) is **GTM Versie 8** ("GA4 reservation_widget_open tracking") gepubliceerd, met exact deze vier objecten als Versie 7→8-delta. Een daaropvolgende schone, niet-Preview productietest (Tag Assistant expliciet "Niet verbonden") toonde GA4 Realtime-ontvangst van zowel `reservation_click` als `reservation_widget_open` (beide = 1). **Nog onopgelost, apart geregistreerd (O-021):** het eerdere, onverklaarde Preview-herlaad-incident blijft UNEXPLAINED/NOT REPRODUCED, ongewijzigd door deze sluiting. **Update 28 augustus 2026 (O-021, Test 6; Challenge Evidence/CR-register.md, CR-009):** de werkruimte-restindicator ("4 wijzigingen / 0 conflicten") is onderzocht via object-voor-object vergelijking van twee read-only exports (Gepubliceerde Versie 8 vs. huidige Default Workspace) — geen inhoudelijke configuratie-afwijking gevonden (Classificatie B); de interne GTM-oorzaak van de restteller blijft **onverklaard** en wordt hier niet gegist. Dit heropent of verzwakt de Production Verified-vaststelling niet. Diagnostische instrumentatie (`console.log`, `window.__gp10`) blijft aanwezig in de listener, bewust niet verwijderd als onderdeel van deze sluiting. `private_dining_enquiry` en `catering_enquiry` blijven ongewijzigd: nog niet gemeten. **`private_dining_enquiry`: bijgewerkt 26 september 2026 (observations/O-026.md) — Production Verified.** Aanvulling op observations/O-025.md (26 september 2026: "Validated in GTM Preview; GA4 receipt observed; production verification outside Preview pending", keten toen 4/6 — historisch juist, niet herschreven). De gepubliceerde GTM-container `GTM-WXH5P6SN` **Versie 9** (Live, geladen door gewone bezoekers zonder `gtm_auth`/`gtm_preview`) bevat de GA4-eventtag voor `private_dining_enquiry` (`G-C29ZMF288W`, geen formulierveld-parameters), de bijbehorende Custom Event-trigger en de Google-tag; weergavenaam van de tag en werkruimte-staat niet vastgesteld. Preview-gedragsbewijs overgenomen uit O-025. Met Tag Assistant/Preview volledig losgekoppeld (bevestigd door de Case Owner) toonde GA4 Realtime na één nieuwe gecontroleerde productie-inzending `private_dining_enquiry` = 1 (alleen ontvangst binnen het Realtime-venster; zie O-026 voor de volledige bewijsgrenzen). **`catering_enquiry`: bijgewerkt 26 september 2026 (observations/O-028.md) — Production Verified.** Aanvulling op observations/O-027.md (26 september 2026: gap geregistreerd, Not Production Verified, keten toen 5/6 — historisch juist, niet herschreven). Website-commit `106016c` (eenmalige serverreceipt, Optie A) gedeployd met hashverificatie; één productie-inzending gaf een servergeautoriseerde succesmelding en precies één `catering_enquiry`-entry in de dataLayer zonder formulier-PII; in GTM Preview vuurde `GA4 - Event - catering_enquiry` eenmaal; **GTM Versie 10** ("EC-002 Catering enquiry") gepubliceerd en zelfstandig bevestigd in de geleverde `gtm.js`; na een productietest buiten Preview toonde GA4 Realtime `catering_enquiry` (count 2, alleen als venstertelling vastgelegd; zie O-028 voor de volledige bewijsgrenzen). **Dit sluit de bredere first-party-meetketen af** (measurement/2026-W34-visibility-brief.md, Prioriteit 3, zes vereiste events) — zes van de zes (`reservation_click`, `phone_click`, `directions_click`, `reservation_widget_open`, `private_dining_enquiry`, `catering_enquiry`) zijn nu Production Verified. De Verdict van HV-INT-003 hieronder blijft ongewijzigd. |
...
| **Update 23 sep 2026 — sitemap-verwerking** | GSC Sitemaps-rapport (datum getoond: 22-09-2026): status **"Sitemapindex is verwerkt"** — **273 ontdekte pagina's**, 0 ontdekte video's. Het eerdere ophaal-/leesprobleem is opgelost. Dit is een telling op sitemapindex-niveau — **niet** geïnterpreteerd als bewijs dat alle 273 pagina's geïndexeerd zijn, en **niet** geïnterpreteerd als bewijs dat `/omakase-utrecht/` specifiek tot de ontdekte/gecrawlde/geïndexeerde pagina's behoort |
| **Update 23 sep 2026 — Omakase-URL herinspectie** | `https://konnichiwa.nl/omakase-utrecht/` opnieuw gecontroleerd via URL-inspectie: **ongewijzigd** t.o.v. 22 sep — "URL is niet geïndexeerd door Google," "URL is onbekend bij Google," geen referring sitemap, geen referring page, last crawl N/A. Geen indexeringsaanvraag gedaan. Zie observations/O-005.md (Update 23 sep 2026) voor het volledige bewijs |
| Eigenaar | Kelvin |
| Implementatiebewijs | GSC Sitemaps-rapport, 22 sep 2026 (owner-observed, screenshot-based); observations/O-005.md (22 sep 2026 update); GSC Sitemaps-rapport + URL-inspectie, 23 sep 2026 (owner-observed); observations/O-005.md (23 sep 2026 update) |
| Externe invloeden | Geen bekende |
| Resultaat | **SITEMAP PROCESSING — PASS** (273 pagina's ontdekt, 0 video's; het ophaal-/leesprobleem is opgelost — de sitemap wordt hiermee **niet met terugwerkende kracht** als "technisch defect" heringedeeld, want dat is nooit de classificatie geweest). **OMAKASE DISCOVERY — PENDING. OMAKASE CRAWL — NOT YET EVIDENCED. OMAKASE INDEXATION — PENDING.** Geen claim dat de Omakase-URL tot de reeds gecrawlde/geïndexeerde pagina's behoort. Geen rangschikkings- of zichtbaarheidseffect wordt geclaimd. Geen causaal verband tussen sitemap-verwerking en Omakase-specifieke ontdekking/indexatie wordt gesteld, in geen van beide richtingen |
| Verdict | **SITEMAP PROCESSING CONFIRMED (273 pagina's ontdekt) — OMAKASE URL DISCOVERY/CRAWL/INDEXATION STILL PENDING (23 sep 2026 herinspectie).** Niet "afgerond/opgelost" als geheel — de sitemap-laag is opgelost, de Omakase-URL-laag blijft een open, ongewijzigde bevinding |
